  ...
  return (
    <div className="min-h-screen bg-background text-gray-800">
      <div className="max-w-2xl mx-auto py-6 px-4">
        <motion.h2 className="text-2xl font-bold mb-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>My Skills</motion.h2>
        ...
        {showAdd ? (
          ...
          >
            ...
            <button className="bg-primary text-white px-4 py-2 rounded mt-2 w-full" type="submit">{loading ? 'Adding...' : 'Add Skill'}</button>
          </motion.form>
        ) : (
                ...
                {skills.map((skill, idx) => (
                  ...
                  >
                    ...
                    <div className="flex gap-4 text-sm mt-2">
                      ...
                      <span>{skill.is_verified ? <span className="text-green-600">Verified</span> : <span className="text-yellow-500">Pending</span>}</span>
                    </div>
                    ...
                    <div className="mt-3">
                      ...
                      <ul className="list-disc ml-6 text-xs text-gray-700">
                        ...
                        {(skill.skill || '').toLowerCase() === 'driving' && <li>Driving License</li>}
                      </ul>
                    </div>
                    <div className="flex gap-2 mt-3">
                      <button
                        ...
                        onClick={() => {
                          if (skill.skill === 'Driving' && !hasDrivingLicense(skill)) {
                            setError('Please upload your Driving License to proceed with assessment.');
                            return;
                          }
                      ...
                      {skill.skill === 'Driving' && !hasDrivingLicense(skill) && (
                        <div className="text-red-500 text-xs mt-1">Driving License upload is mandatory for Driving skill.</div>
                      )}
                      <button
                    ...
                    {skill.assessment && <div className="mt-2 text-green-600 text-xs">Assessment assigned! Check your email/SMS for the link.</div>}
                  </motion.div>
                ))}
              </div>
            )}
            <button className="w-full mt-4 py-2 bg-primary text-white rounded-lg font-semibold" onClick={() => setShowAdd(true)}>+ Add Another Skill</button>
          </motion.div>
        )}
        ...
        <div className="mt-10 bg-white rounded-xl shadow p-4 flex flex-col border-l-8 border-primary max-w-3xl mx-auto">
          <h2 className="text-lg font-bold mb-2 text-primary">Documents</h2>
          <form onSubmit={handleDocUpload} className="mb-4">
            ...
            >
              ...
              {docFile ? (
                ...
                <div className="text-gray-400">Drag & drop a photograph here, or click to select</div>
              )}
            </div>
            <div className="flex gap-2 items-center mb-2">
              ...
              <select value={docType} onChange={handleDocTypeChange} className="border rounded px-2 py-1">
                ...
                {hasDriving && <option value="license">Driving License</option>}
                <option value="certificate">Skill Certificate</option>
              </select>
              <button type="submit" className="ml-2 px-3 py-1 bg-primary text-white rounded font-semibold shadow" disabled={docUploading}>{docUploading ? 'Uploading...' : 'Upload'}</button>
            </div>
            {docError && <div className="text-red-500 text-xs text-center mb-2">{docError}</div>}
          </form>
          <div className="mb-2">
            <h3 className="font-semibold mb-1 text-sm">Uploaded Documents</h3>
            {documents.length === 0 && <div className="text-gray-500 text-xs">No documents uploaded yet.</div>}
            <div className="grid grid-cols-2 gap-2">
              {documents.map(doc => (
                ...
                >
                  ...
                  <div className={`text-xs ${doc.verified ? 'text-green-600' : 'text-yellow-500'}`}>{doc.verified ? 'Verified' : 'Pending'}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {missingDocs.length > 0 && (
            <div className="bg-yellow-100 border-l-4 border-yellow-500 p-2 rounded mb-2">
              <div className="font-semibold text-yellow-700 mb-1 text-xs">Please upload these to get verified:</div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-sm text-center">
            ...
            <div className="text-gray-700 mb-4">Are you sure you want to delete this skill? This action cannot be undone.</div>
