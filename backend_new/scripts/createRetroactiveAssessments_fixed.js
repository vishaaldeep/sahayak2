...
async function createAssessmentRecord(userId, skillId, jobId, assignedBy, models) {
  try {
    const { Assessment, AssessmentQuestion, Skill } = models;
...
async function updateUserSkillAssessmentStatus(userId, skillId, models) {
  try {
    const { UserSkill } = models;
...
async function createRetroactiveAssessments() {
  try {
    ...
    const assessmentJobs = await models.Job.find({ 
    ...
    for (const job of assessmentJobs) {
      ...
      const hiredApplications = await models.UserApplication.find({
        job_id: job._id,
        status: 'hired'
    ...
    for (const job of assessmentJobs) {
      for (const skill of job.skills_required) {
        const questionCount = await models.AssessmentQuestion.countDocuments({ skill_id: skill._id });
        if (questionCount === 0) {
  ...
  try {
    ...
    for (const job of assessmentJobs) {
      ...
      for (const application of hiredApplications) {
        ...
        for (const skill of job.skills_required) {
          const existingAssessment = await models.Assessment.findOne({
